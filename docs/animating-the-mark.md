...
There is no `prefers-color-scheme` hook available to an `<img>`-loaded SVG, so pick mid-tone
colors that survive both light and dark themes — pure white snow vanishes on light mode,
pastels vanish on dark.
...
the consumer pick, but picking requires a hook the `<img>` path does not give you. The
only single-file answer is to stop being transparent — bring your own background, which
is what `assets/anime/chan-monkey-live-on-white.svg` does (generated by
